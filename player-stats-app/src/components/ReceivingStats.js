...
const ReceivingStats = () => {
  ...
  const handleSearch = async () => {
    ...
    await axios
      .get(`http://localhost:8080/api/stats/player/${searchTerm}/predict/receiving`)
      .then((response) => {
  ...
  return (
    <div className="overflow-x-auto">
          ...
          <tbody>
            {stats.map((stat, index) => (
              <tr key={index}>
                ...
                <td className="border px-4 py-2">{stat.tgt || "N/A"}</td>
                <td className="border px-4 py-2">{stat.rec || "N/A"}</td>
                <td className="border px-4 py-2">{stat.yds || "N/A"}</td>
                <td className="border px-4 py-2">{stat.yardsPerReception || "N/A"}</td>
                ...
                <td className="border px-4 py-2">{stat.receptionsPerGame || "N/A"}</td>
                <td className="border px-4 py-2">{stat.yardsPerGame || "N/A"}</td>
                <td className="border px-4 py-2">{stat.catchPercent || "N/A"}</td>
                <td className="border px-4 py-2">{stat.yardsPerTarget || "N/A"}</td>
      ...
      {!loading && stats.length === 0 && !error && <p className="text-center">No data available. Try another player.</p>}
      ...
      <table className="table-auto w-full border-collapse border border-gray-300">
          ...
          <tbody>
              <tr key="pred">
                ...
                <td className="border px-4 py-2">{predStats.PredictedTargets || "N/A"}</td>
                <td className="border px-4 py-2">{predStats.PredictedTouchdowns || "N/A"}</td>
                <td className="border px-4 py-2">{predStats.PredictedReceptions || "N/A"}</td>
              </tr>
            </tbody>
        </table>
    </div>
